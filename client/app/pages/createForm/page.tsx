...
function CreateForm() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#f9f9fc] px-4 py-10">
      ...
      <div className="w-full max-w-xl rounded-sm border border-gray-200 shadow-sm bg-white p-8 space-y-6">
        <div>
          ...
          <Select value={formData.outlet} onValueChange={(val) => handleInputChange("outlet", val)}>
            ...
            <SelectContent className="bg-white border border-[#2d2d2d]/50 shadow-md">
              {outlets.map((outletVal) => (
                <SelectItem key={outletVal} value={outletVal} className="cursor-pointer hover:bg-[#2F27CE]/10">
                  {outletVal.replace("_", " ")}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="grid grid-cols-2 gap-y-4">
          ...
          {["wine", "beer", "juice"].map((type) => (
            <div key={type} className="col-span-3 flex items-center justify-center gap-4">
              <Label className="w-32 text-[#2d2d2d] font-normal">{type.charAt(0).toUpperCase() + type.slice(1)}</Label>
              <Input
                type="text"
